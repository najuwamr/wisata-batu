...
@section('content')
<style>
    nav { position: fixed !important; top: 0 !important; z-index: 9999 !important; }
    body { padding-top: 5rem !important; }
</style>

<div class="max-w-2xl mx-auto bg-white p-2 sm:p-4 md:p-6 rounded-xl shadow-lg">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    btn.addEventListener('click', async function() {
        ...
        try {
            // Clone elemen untuk diproses
            const clone = etiket.cloneNode(true);
            clone.style.position = 'absolute';
            clone.style.left = '-9999px';
            clone.style.top = '0';
            document.body.appendChild(clone);

            // Override semua warna dengan RGB/HEX untuk kompatibilitas html2canvas
            ...
            alert('Terjadi kesalahan saat membuat e-ticket. Error: ' + error.message);
        }

        btn.disabled = false;
        btn.innerHTML = '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M7 10l5 5m0 0l5-5m-5 5V4"/></svg> Unduh E-Ticket';
    });
});
</script>
